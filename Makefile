...
XCRUN := $(shell which xcrun 2>/dev/null)
...
.PHONY: all train_gpt2 test_gpt2 train_gpt2cu test_gpt2cu train_gpt2fp32cu test_gpt2fp32cu train_gpt2_metal
...
ifeq ($(XCRUN),)
	$(info xcrun not found. run xcode-select --install to enable Metal builds)
else
    $(info xcrun found, including Metal builds)
    TARGETS += train_gpt2_metal
endif	
...
clean:
	rm -f train_gpt2 test_gpt2 train_gpt2cu train_gpt2fp32cu test_gpt2cu test_gpt2fp32cu train_gpt2_metal gpt2.air default.metallib
